...
def fuzzifier1(value):
    v = int(value)
    classes = ['thin', 'normal', 'thick', 'very thick']
    class1 = random.uniform(0, min(1, max(0, 1.25 - 0.5 * v)))
    class2 = random.uniform(0, min(min(max(-0.25 + 0.5 * v, 0), 1), max(min(3.25 - 0.5 * v, 1), 0)))
    class3 = random.uniform(0, min(min(max(-1.75 + 0.5 * v, 0), 1), max(min(4.75 - 0.5 * v, 1), 0)))
    class4 = random.uniform(0, min(max(-3.25 + 0.5 * v, 0), 1))
...
def main():
    ...
    for row, data in enumerate(all_records[1:]):
        worksheet.update_cell(row + 2, len(data), fuzzifier2(value=data[headers.index('clump_thickness')]))
        time.sleep(1.1)
